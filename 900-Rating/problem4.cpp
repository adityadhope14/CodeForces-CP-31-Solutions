...
void solve() {
    ...
    long long ans = b;   // Initial time before using any tools

    for (int i = 0; i < n; i++) {
        long long x;
        cin >> x;

        // If x <= a-1, this tool gives exactly x extra seconds.
        // If x > a-1, because the timer cannot exceed 'a',
        // the maximum extra time this tool can contribute is (a-1).
        ans += min(x, a - 1);
    }
